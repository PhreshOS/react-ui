...
export interface CheckboxProps extends Omit<CheckboxFieldProps, ControlOverrides | "isReadOnly" | "isSelected" | "defaultSelected" | "isIndeterminate">, ControlProps, FieldProps {
...
export const Checkbox = forwardRef<HTMLDivElement, CheckboxProps>(function Checkbox({
    ...
    checked, defaultChecked, indeterminate, size, color, style, ...properties
}, ref) {
    ...
    return <CheckboxField {...properties} ref={ref} isDisabled={disabled} isRequired={required} isInvalid={invalid}
        ...
        <CheckboxButton style={state => toggleStyle(theme, state.isDisabled, state.isReadOnly)}>
            {state => <>
                <ToggleIndicator kind="checkbox" theme={theme} selected={state.isSelected} indeterminate={state.isIndeterminate}
