...
export type SingleSelectionProps = SingleStringSelectionProps & Readonly<{ selectionMode: "single" }>

export type MultipleSelectionProps = MultipleStringSelectionProps & Readonly<{ selectionMode: "multiple" }>
...
export function ariaSelection(properties: AnySelection, fallbackMode: "none" | "single" | "multiple") {
  ...
  const onChange = properties.onChange as ((value: unknown) => void) | undefined
  const onSelectionChange = onChange === undefined || mode === "none"
    ? undefined
    : (selection: Selection) => onChange(mode === "multiple" ? multipleSelection(selection) : singleSelection(selection))
