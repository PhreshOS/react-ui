...
const RadioStyle = createContext<Pick<ControlProps, "size" | "color" | "disabled">>({})

export interface RadioGroupProps extends Omit<AriaRadioGroupProps, ControlOverrides | "isReadOnly">, ControlProps, FieldProps {
    readonly children: ReactNode
    readonly readOnly?: boolean
}

export interface RadioProps extends Omit<RadioFieldProps, ControlOverrides>, ControlProps, Pick<FieldProps, "label" | "description"> {}
...
export const RadioGroup = forwardRef<HTMLDivElement, RadioGroupProps>(function RadioGroup({
    ...
    size, color, style, orientation = "vertical", ...properties
}, ref) {
    ...
    return <AriaRadioGroup {...properties} ref={ref} isDisabled={disabled} isReadOnly={readOnly}
        ...
        <RadioStyle.Provider value={{ size, color, disabled }}>
...
export const Radio = forwardRef<HTMLDivElement, RadioProps>(function Radio({ label, description, disabled, size, color, style, ...properties }, ref) {
    ...
    return <RadioField {...properties} ref={ref} isDisabled={disabled} style={state => fieldStyle(theme, state.isDisabled && !inherited.disabled, style)}>
        <RadioButton style={state => toggleStyle(theme, state.isDisabled, state.isReadOnly)}>
            {state => <>
                <ToggleIndicator kind="radio" theme={theme} selected={state.isSelected} focused={state.isFocusVisible} invalid={state.isInvalid}
