...
const RadioStyle = createContext<Pick<ControlProps, "size" | "color" | "disabled"> & MaterialOverrides & ShadowOverrides>({})

export interface RadioGroupProps extends Omit<AriaRadioGroupProps, ControlOverrides | "isReadOnly">, ControlProps, FieldProps, MaterialOverrides, ShadowOverrides {
    readonly children: ReactNode
    readonly readOnly?: boolean
}

export interface RadioProps extends Omit<RadioFieldProps, ControlOverrides>, ControlProps, Pick<FieldProps, "label" | "description">, MaterialOverrides, ShadowOverrides {}
...
export const RadioGroup = forwardRef<HTMLDivElement, RadioGroupProps>(function RadioGroup({
    ...
    size, color, style, material, shadow, orientation = "vertical", ...properties
}, ref) {
    ...
    return <AriaRadioGroup {...properties} ref={ref} isDisabled={disabled} isReadOnly={readOnly}
        ...
        <RadioStyle.Provider value={{ size, color, disabled, material, shadow }}>
...
export const Radio = forwardRef<HTMLDivElement, RadioProps>(function Radio({ label, description, disabled, size, color, style, material, shadow, ...properties }, ref) {
    ...
    return <RadioField {...properties} ref={ref} isDisabled={disabled} style={state => fieldStyle(theme, state.isDisabled && !inherited.disabled, style)}>
        <RadioButton style={state => toggleStyle(theme, state.isDisabled, state.isReadOnly)}>
            {state => <>
                <ToggleIndicator kind="radio" material={material ?? inherited.material} shadow={shadow ?? inherited.shadow} theme={theme} selected={state.isSelected} focused={state.isFocusVisible} invalid={state.isInvalid}
